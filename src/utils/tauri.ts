...
export class TauriAPI {
    ...
    async initialize(): Promise<boolean> {
        try {
            ...
            if (!this.isTauri) {
                console.warn('Not running in Tauri environment - some features will be disabled');
    ...
    async invoke<T>(command: string, args?: Record<string, any>): Promise<T> {
        if (!this.initialized) {
            throw new Error('Tauri API not initialized - make sure you\'re running in the Tauri app');
        }
    ...
    async listen<T>(event: string, handler: (event: { payload: T }) => void) {
        ...
        try {
            ...
            console.warn('Event listening failed, possibly due to permissions:', error);
            return () => { }; // 返回一个空的取消函数
        }
    }
