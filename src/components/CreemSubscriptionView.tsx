...
const CreemSubscriptionView: React.FC<CreemSubscriptionViewProps> = ({
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            alert(`${t('creemSubscription.purchaseFailed')}: ${error}`);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const getStatusText = (status: string) => {
        switch (status) {
            case 'PAID':
                return t('creemSubscription.paymentSuccess');
            case 'CANCELLED':
                return t('creemSubscription.paymentFailed');
            case 'EXPIRED':
                return t('creemSubscription.sessionExpired');
            case 'PENDING':
                return t('creemSubscription.pendingPayment');
            default:
                return t('creemSubscription.unknownStatus');
        }
    };
                        ...
                        <div className='w-full max-w-md p-4 bg-blue-50 rounded-lg border border-blue-200'>
                            <div className='flex items-center justify-between mb-2'>
                                <span className='text-sm font-medium text-gray-700'>
                                    {t('creemSubscription.paymentStatus')}
                                </span>
                                <span
                                    className={`px-2 py-1 rounded text-xs font-medium ${paymentStatus.userPackages[0].status === 'PAID'
                            ...
                            {paymentStatus.userPackages[0].status === 'PAID' && (
                                <div className='text-sm text-green-700'>
                                    ✅ {t('creemSubscription.paymentSuccessAmount', {
                                        amount: (paymentStatus.userPackages[0].amount / 100).toFixed(2)
                                    })}
                                </div>
                            )}
                            ...
                            {isPolling && (
                                <div className='flex items-center text-blue-600 text-sm mt-2'>
                                    ...
                                    {t('creemSubscription.checkingPayment')}
                                </div>
                            )}
                        </div>
                    </div>
                ...
                >
                    <div className='p-5'>
                        <h2 className='text-2xl font-semibold text-gray-900 text-center'>
                            {t('creemSubscription.title')}
                        </h2>
                        <p className='mt-2 text-sm text-gray-500 text-center'>
                            {t('creemSubscription.subtitle')}
                        </p>
                        ...
                        >
                            {isLoading ? (
                                <div className='flex items-center justify-center'>
                                    ...
                                    {t('creemSubscription.processing')}
                                </div>
                            ) : (
                                t('creemSubscription.purchaseNow')
                            )}
                        </button>
                    </div>
                </div>
