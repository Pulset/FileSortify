...
const SubscriptionView: React.FC = () => {
  ...
  const handleSubscribe = async (plan: 'monthly' | 'yearly') => {
    try {
      ...
      alert(
        `正在处理${
          plan === 'monthly' ? '月度' : '年度'
        }订阅购买，请在App Store对话框中完成购买。`
      );
    } catch (error) {
      addLog(`❌ 启动订阅失败: ${error?.message}`, 'error');
      alert(`启动订阅失败: ${error?.message}`);
    }
  };
  ...
  const handleRestorePurchases = async () => {
    try {
      ...
      addLog(`❌ 恢复购买失败: ${error?.message}`, 'error');
      alert(`恢复购买失败: ${error?.message}`);
    }
  };
    ...
    if (!subscription) {
      return (
        <div className='loading'>
          <div className='spinner'></div>加载订阅信息失败
        </div>
      );
    }
    ...
    if (subscription.status === 'Active') {
      const daysRemaining = subscription.subscription_end_date
        ? Math.max(
            0,
            Math.floor(
              (new Date(subscription.subscription_end_date).getTime() -
                Date.now()) /
                (24 * 60 * 60 * 1000)
            )
          )
      ...
      return (
        <div className='subscription-card active'>
          ...
          <div className='subscription-details'>
            {subscription.plan === 'Monthly' ? '月度' : '年度'}订阅 · 剩余{' '}
            {daysRemaining} 天
          </div>
